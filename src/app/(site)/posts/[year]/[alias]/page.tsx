...
import { SlateNode, SlateReactPresentation } from "@/components/RichText";
...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (data.entry.__typename === "PostBase") {
    return {
...
export default async function Post({ params }: PageProps) {
  ...
  if (
    data.entry.__typename !== "PostRichtext" &&
    data.entry.__typename !== "PostMarkdown"
  ) {
    notFound();
  }
  ...
  return (
    <main className="post">
      ...
      {post.__typename === "PostRichtext" &&
        post.bodyRich?.map((val: { body?: unknown } | null, idx: number) =>
          val ? (
            <article key={idx}>
              <SlateReactPresentation value={val.body as SlateNode[]} />
            </article>
          ) : null
        )}
      {post.__typename === "PostMarkdown" && post.body ? (
